          ...
          >
            <div className="p-6 border-b border-on-surface/10 flex justify-between items-center bg-on-surface/[0.02]">
              <div>
                ...
                <p className="text-xs text-on-surface/50 mt-1 uppercase tracking-widest font-black">{book?.title}</p>
              </div>
              <button onClick={onClose} className="p-2 hover:bg-on-surface/5 rounded-lg transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-80 p-6 border-b md:border-b-0 md:border-r border-on-surface/10 bg-on-surface/[0.02]">
                ...
                <form onSubmit={handleAddCopy} className="space-y-4">
                  <div>
                    <label className="text-[10px] font-bold text-on-surface/50 uppercase block mb-1.5">Mã vạch (Barcode) *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-on-surface/50 uppercase block mb-1.5">Vị trí (Location)</label>
                    <div className="relative group">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-on-surface/50 uppercase block mb-1.5">Trạng thái ban đầu</label>
                    <select
                  ...
                  >
                    {adding ? <div className="w-4 h-4 border-2 border-on-surface/30 border-t-white rounded-full animate-spin"></div> : <Save className="w-4 h-4" />}
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                {loading ? (
                  <div className="flex flex-col gap-3">
                    {[1, 2, 3].map(i => <div key={i} className="h-16 bg-on-surface/5 rounded-xl animate-pulse"></div>)}
                  </div>
                ) : copies.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {copies.map((copy) => (
                      <div key={copy._id} className="p-4 rounded-xl bg-on-surface/[0.02] border border-on-surface/10 flex items-center justify-between group hover:border-on-surface/15 transition-all">
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                             <div className="flex items-center gap-2 mb-1">
                               <p className="font-bold text-on-background line-clamp-1">{copy.barcode}</p>
