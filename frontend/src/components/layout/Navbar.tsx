  ...
  return (
    <nav className="glass-nav px-6 py-4 flex items-center justify-between sticky top-0 z-50">
      ...
      <div className="hidden md:flex items-center gap-8">
        ...
        <Link to="/search" className="text-slate-300 hover:text-white flex items-center gap-1.5 transition-colors font-medium">
          <Search className="w-4 h-4" /> Tìm kiếm
        </Link>
        {(user?.role === 'admin' || user?.role === 'librarian' || user?.role === 'superadmin') && (
          <Link to="/admin" className="text-primary hover:text-primary-light flex items-center gap-1.5 transition-colors font-bold">
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            ...
            <div className="flex gap-2">
              <Link to="/login" className="text-slate-300 hover:text-white px-4 py-2 font-medium">
                Đăng nhập
              </Link>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 right-0 bg-background/95 backdrop-blur-xl border-b border-white/5 p-6 md:hidden z-50 flex flex-col gap-6"
          >
            <div className="flex flex-col gap-4">
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-slate-300 hover:text-white flex items-center gap-3 transition-colors font-medium text-lg">
              ...
              {user ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <User className="w-10 h-10 p-2 bg-white/5 rounded-full text-slate-400" />
                    <div>
                      <p className="text-sm font-bold text-white">{user.profile.firstName} {user.profile.lastName}</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Link to="/login" onClick={() => setIsMenuOpen(false)} className="text-center py-3 text-slate-300 font-medium">
                    Đăng nhập
                  </Link>
                  <Link to="/register" onClick={() => setIsMenuOpen(false)} className="premium-button text-center">
                    Đăng ký
                  </Link>
                </div>
              )}
