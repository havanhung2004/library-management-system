  ...
  body: Joi.object().keys({
    copyId: Joi.string().custom((value, helpers) => {
      if (!value.match(/^[0-9a-fA-F]{24}$/)) {
        return helpers.message({ custom: '"copyId" must be a valid mongo id' });
      }
      return value;
    }),
...
const returnBook = {
  params: Joi.object().keys({
    loanId: Joi.string().required().custom((value, helpers) => {
      if (!value.match(/^[0-9a-fA-F]{24}$/)) {
        return helpers.message({ custom: '"loanId" must be a valid mongo id' });
      }
      return value;
    }),
  }),
